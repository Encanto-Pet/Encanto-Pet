...
<head>
    ...
    <title>Editar Produto - Encanto Pet</title>
...
<main class="main">
    ...
    <section class="content">
        <div class="header">
            <div><h1>Editar produto <span style="color:var(--yellow)">✏️</span></h1><p class="subtitle">Atualize as informações do produto mantendo a prateleira organizada.</p></div>
        ...
        @endif
        @php
            $selectedCategory = old('category', array_key_exists($product->category, $categoryOptions) ? $product->category : 'outros');
        @endphp
        <div class="form-layout">
            <form class="product-form" action="{{ url('/product/update/' . $product->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="redirect_to" value="{{ route('admin.dashboard', ['section' => 'produtos']) }}">
                <label class="field"><span>Produto</span><input type="text" name="name" id="adminProductName" value="{{ old('name', $product->name) }}" required></label>
                <label class="field"><span>Descrição</span><textarea name="description" id="adminProductDescription" required>{{ old('description', $product->description) }}</textarea></label>
                <label class="field"><span>Preço</span><input type="number" name="price" id="adminProductPrice" value="{{ old('price', $product->price) }}" step="0.01" min="0.01" required></label>
                <label class="field"><span>Estoque</span><input type="number" name="stock" value="{{ old('stock', $product->stock) }}" min="0" step="1" required></label>
                <label class="field">
                    <span>Categoria</span>
                ...
                <div class="actions"><button class="save-btn" type="submit">Salvar edição</button><a class="back-link" href="{{ route('admin.dashboard', ['section' => 'produtos']) }}">Voltar</a></div>
            </form>
            <aside class="preview-card" aria-label="Prévia do produto">
                <div class="preview-image"><button type="button" aria-label="Anterior">‹</button><img id="adminPreviewImage" src="{{ $product->image ? asset('storage/' . $product->image) : asset('assets/img/cachorro-feliz.svg') }}" alt="Prévia"><button type="button" aria-label="Próximo">›</button></div>
                <div class="preview-info"><h2 id="adminPreviewName">{{ $product->name }}</h2><strong class="preview-price" id="adminPreviewPrice">R$ {{ number_format($product->price, 2, ',', '.') }}</strong><div class="stars">★★★★<span>★ (131)</span></div><small class="category-label">Categoria</small><p class="category-value" id="adminPreviewCategory">{{ $categoryOptions[$selectedCategory] }}</p></div>
            </aside>
        </div>
    </section>
</main>
